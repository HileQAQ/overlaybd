...
#include "overlaybd/cache/pool_store.h"
...
class CachedGzFs : public ICachedFileSystem {
public:
    CachedGzFs(IFileSystem *srcFs, ICachePool *fileCachePool, size_t pageSize, size_t refillUnit,
               IOAlloc *allocator)
    ...
    IFile *open(const char *pathname, int flags, mode_t mode) {
        ...
        auto ret = new_cached_gzfile(srcFile, cache_store, pageSize_, refillUnit_, allocator_, this);
...
ICachedFileSystem *new_cached_gzfs(IFileSystem *src, ICachePool *pool, uint64_t pageSize,
                                   uint64_t refillUnit, IOAlloc *allocator) {
    if (!allocator) {
        allocator = new IOAlloc;
    }
    return new ::Cache::CachedGzFs(src, pool, pageSize, refillUnit, allocator);
}
} // namespace FileSystem
