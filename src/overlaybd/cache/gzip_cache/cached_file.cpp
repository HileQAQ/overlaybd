...
#include "src/overlaybd/cache/pool_store.h"


namespace Cache {

const off_t kMaxPrefetchSize = 1024 * 1024;

CachedGzFile::CachedGzFile(photon::fs::IFile *src_file, FileSystem::ICacheStore *cache_store,
...
CachedGzFile::~CachedGzFile() {
    cache_store_->release();
    delete src_file_;
}

IFileSystem *CachedGzFile::filesystem() {
    return fs_;
}

ssize_t CachedGzFile::pread(void *buf, size_t count, off_t offset) {
    struct iovec v {
        buf, count
    };
...
ssize_t CachedGzFile::preadv(const struct iovec *iov, int iovcnt, off_t offset) {
...
ssize_t CachedGzFile::prefetch(size_t count, off_t offset) {
...
ssize_t CachedGzFile::preadvInternal(const struct iovec *iov, int iovcnt, off_t offset) {
    if (offset < 0) {
...
ssize_t CachedGzFile::pwrite(const void *buf, size_t count, off_t offset) {
    struct iovec v {
...
ssize_t CachedGzFile::pwritev(const struct iovec *iov, int iovcnt, off_t offset) {
    if (offset >= size_) {
        return 0;
    }
...
int CachedGzFile::fiemap(struct fiemap *map) {
    errno = ENOSYS;
    return -1;
}

int CachedGzFile::query(off_t offset, size_t count) {
    auto ret = cache_store_->queryRefillRange(offset, count);
    return ret.second;
}

int CachedGzFile::fallocate(int mode, off_t offset, off_t len) {
    if (len == -1) {
...
int CachedGzFile::fstat(struct stat *buf) {
    return src_file_ ? src_file_->fstat(buf) : -1;
}

int CachedGzFile::close() {
    if (src_file_) {
...
ssize_t CachedGzFile::read(void *buf, size_t count) {
    struct iovec v {
        buf, count
    };
    return readv(&v, 1);
...
ssize_t CachedGzFile::readv(const struct iovec *iov, int iovcnt) {
...
ssize_t CachedGzFile::write(const void *buf, size_t count) {
    struct iovec v {
        const_cast<void *>(buf), count
    };
    return writev(&v, 1);
...
ssize_t CachedGzFile::writev(const struct iovec *iov, int iovcnt) {
...
std::string_view CachedGzFile::get_pathname() {
...
ICachedFile *new_cached_gzfile(IFile *src, ICacheStore *store, uint64_t pageSize, uint64_t refillUnit,
    ...
    return new CachedGzFile(src, store, st.st_size, pageSize, refillUnit, allocator, fs);
}
